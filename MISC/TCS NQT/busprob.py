# A City Bus is a Ring BusStops Bus which runs in circular fashion.That is , Bus once starts at the Source Bus Stop, halts at each Bus Stop in its BusStops and at the end it reaches the Source Bus Stop again.
# If there are n  number of Stops and if the bus starts at Bus Stop 1, then after nth Bus Stop, the next stop in the BusStops will be Bus Stop number 1 always.
# If there are n stops, there will be n paths.One path connects two stops. Distances ( in meters) for all paths in Ring BusStops is given in array Path[] as given below:
...
def getFare(src, dest):
    ...
    if not (src in BusStops[0] and dest in BusStops[0]):
        print("Invalid Input")
        exit()

    if BusStops[0].index(src) < BusStops[0].index(dest):
        for i in range(BusStops[0].index(src),BusStops[0].index(dest) + 1):

            fares += BusStops[1][i]

    elif BusStops[0].index(dest) < BusStops[0].index(src):
        for i in range(BusStops[0].index(src) + 1,len(BusStops[0])):

            fares += BusStops[1][i]

        for i in range(0,BusStops[0].index(dest) + 1):

            fares += BusStops[1][i]

    return float(math.ceil(fares * 0.005))
